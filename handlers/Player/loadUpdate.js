...
module.exports = async (client) => {
    ...
    client.UpdateQueueMsg = async function (queue) {
        const CheckDB = await db.has(`${queue.textChannel.guild.id}_${client.user.id}`);
        if(!CheckDB) return;

        const data = await db.get(`${queue.textChannel.guild.id}_${client.user.id}`);
        if (data.setup_enable === false) return;

        // forum correct
        const forum = await client.channels.cache.get(queue.textChannel.id);
        if (!forum) return;
        if (forum.id != data.setup_td) return;

        const playMsg = await forum.messages.fetch(data.setup_msg, { cache: false, force: true });
        if (!playMsg) return;
